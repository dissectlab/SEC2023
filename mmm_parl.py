...
if not CAMERA_SENSOR_DB_DIRECTORY:
    CAMERA_SENSOR_DB_DIRECTORY = "/home/edge/openMVG/src/software/SfM" + "/../../openMVG/exif/sensor_width_database"


PRESET = {'SEQUENTIAL': [0, 1, 2, 3, 4, 5, 11, 12],
          'SEQUENTIAL_FIX': [1, 2, 3, 8, 11, 12],
          'MVG_FIX': [1, 2, 3, 8, 11],
          "MVS_FIX": [12],
          'GLOBAL': [0, 1, 2, 3, 4, 6, 11, 12, 13, 14, 15],
    ...
    def __init__(self):
        self.steps_data = [
            ...
            ["Compute features",             # 1
             ...
             ["-i", "%matches_dir%"+FOLDER_DELIM+"sfm_data.json", "-o", "%matches_dir%", "-m", "SIFT", "-p", "HIGH", "-n", "8"]],
            ["Compute pairs",                # 2
            ...
            ["Structure from Known Poses",   # 8
             ...
             ["-i", "%matches_dir%"+FOLDER_DELIM+"sfm_data.json", "-m", "%matches_dir%",  "-o", "%reconstruction_dir%"+FOLDER_DELIM+"sfm_data.bin"]], #"-f", "%matches_dir%"+FOLDER_DELIM+"matches.f.bin",
            ...
            ["Densify point cloud",          # 12
             ...
             ["scene.mvs", "--dense-config-file", "Densify.ini", "--resolution-level", "0", "--number-views", "8", "-w", "\"%mvs_dir%\""]], #, "--tasks", "%tasks%", "--task-number", "%task_number%", "--do-fuse", "%do_fuse%", "--skipList", "%skip%"
...
PARSER.add_argument('--tasks', default="[000]-[004]",
                    help="task IDs")
PARSER.add_argument('--task_number', default="5",
                    help="number of tasks")
PARSER.add_argument('--do_fuse', default="1",
                    help="do_fuse")
PARSER.add_argument('--skip', default="[00000]",
                    help="skip")
...
for cstep in CONF.steps:
    ...
    if not DEBUG:
        ...
        try:
            pStep = subprocess.Popen(cmdline, stdout=subprocess.DEVNULL, stderr=subprocess.STDOUT) #
            pStep.wait()
            if pStep.returncode != 0:
                time_info[STEPS[cstep].info] = time.time() - start
                if STEPS[cstep].info == "Export to openMVS":
                    time_info["openMVG"] = time.time() - start_t
                break
...
print(json.dumps(time_info, indent=4))
