...
class Scheduler:
    def __init__(self, th=0.06, quality=0.7, start=245, end=2245):
        self.done = False
        ...
        # f1-res
        self.f1_res = (1.513081102759687, 5.485489453291869, 0.9630077817664481)
        #####################
    ...
    def profile(self):
        ...
        results = [(1.0, 1.0), (0.48, 0.9571), [(0.14, 0.8782), (0.06, 0.7833)]]
    ...
    def task_selection(self, start_timestamp, end_timestamp):
        ...
        while True:
            ...
            if len(self.reconstructed_ids) < 20 and self.timestamp <= self.end:
                ...
                for image_dir in dirs:
                    src.append(os.path.join('handshake', image_dir, str(self.most_recent_3d_id) + ".jpg"))
                    dest.append(os.path.join('handshake', image_dir, img_file_name))
    ...
    def batch_processing(self, start, end, scheduled, assignments, resolution, th):
        ...
        while time.time() - start_t < 3:  # Abr 3 Fix self.init_tp
            # print("ct wait")
            time.sleep(0.2)
