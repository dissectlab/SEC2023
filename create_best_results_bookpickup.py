...
parser.add_argument('--parameter', type=str, default='data/parameter/sfm_data_book.json',  #
                    help="the directory which contains the pictures set.")
...
class Server:
    ...
    def batch_processing(items):
        ...
        if t_resolution < 1.0:
            ...
            sfm["intrinsics"][0]["value"]["ptr_wrapper"]["data"]["focal_length"] = 2638.257231688807 * t_resolution
            sfm["intrinsics"][0]["value"]["ptr_wrapper"]["data"]["principal_point"] = [
                1047.18354216425 * t_resolution,
                535.3155986293243 * t_resolution,
            ]
        ...
        #             "data/gt_book/" + str_timestamp + "_scene_dense.ply")
        #shutil.rmtree(os.path.join(args.output_dir, str_timestamp + "_output"))
        shutil.copy2("data/gold_results_1.0/" + str_timestamp + "_output/mvs/scene_dense.ply",
                     "data/gt_book_f_test/" + str_timestamp + "_" + str(
                         int(t_resolution * 100)) + "_scene_dense.ply")
        shutil.rmtree(os.path.join(args.output_dir, str_timestamp + "_output"))
