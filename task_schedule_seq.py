...
class Scheduler:
    def __init__(self, th=0.06, quality=0.9, start=245 + 100, end=2245 + 100):
        self.done = False
        ...
        # f1-res
        self.f1_res = (1.3690845352745729, 4.812264580079275, 0.956047986254025)
        #####################
        ...
        for v in [1, 3, 5, 10]:
            ...
            for window in ws:
                ...
                for i in range(int(2000/ window)):
                    f1, feq, x1, x2, _, _, _, z = solver_long_tail(v, self.quality_req, finished_q, window, server_paras, diff_th,
                                                                   f1_res, f1_d)
    ...
    def profile(self):
        ...
        items = [
            (0.005, self.timestamp, self.window, self.servers[0]["socket"]),
        ...
        self.s_error.append((x1 + x2) / self.window)
        self.pre_quality = f1
    ...
    def save_d_f_score(self, score, resolution, th):
        ...
        print(f"add resolution={resolution}, th={th}, r_score={raw_score}, f_score={score}, r_score={(c - a * np.exp(-b * resolution))}")
    ...
    def schedule(self, paras):
        ...
        while True:
            ...
            break
        print(logger() + f"ip {ip} is done in {info['time']}/+{round(time.time() - start_t, 4)}, networking={round(time.time() - start_t - info['time'], 4)}")
        return info["status"]
    ...
    def p2(self, th, resolution):
        ...
        start_new_thread(self.batch_processing, (self.online_start_timestamp,
                                                 ...
                                                 resolution,
                                                 th))
        print(logger() + f"resolution = {resolution}, pred_tp_length = {pre_tp_length}, pred_next_tp_tasks = {min(self.window, int(pre_tp_length * self.dd_speed))}")
    ...
    def p1(self, pre_tp_length, pre_number_of_tasks):
        ...
        """
        print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
        if len(self.long_term_f) > 0 and np.abs(self.long_term_f[-1] - self.quality_req) <= 0.025:
            # feq = np.average(self.feqs[-5:])
            self.init_tp = pre_tp_length
            # self.interval = int(min(2, max(1, 0.5/feq)))
        else:
            self.init_tp = max(5, min(pre_tp_length, 10))

        self.intervals.append(self.interval)
        self.window = int(self.dd_speed * self.init_tp * self.interval)
        print(f"interval = {self.interval}")
        print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
        """
    ...
    def task_selection(self, start_timestamp, end_timestamp):
        ...
        while True:
            ...
            while self.in_eva:
                time.sleep(0.1)
            ############################
            #if timestamp - self.online_start_timestamp <= self.window and len(self.reconstructed_ids) < 20:
            #if timestamp - self.online_start_timestamp <= self.window:

            """
            d_ = False 
            if len(self.long_term_f) > 0 and np.abs(self.long_term_f[-1] - self.quality_req) <= 0.02:
                max_task = 10
            """
            ...
            if len(self.reconstructed_ids) < 20:
                ...
                for image_dir in dirs:
                    src.append(os.path.join('walk', image_dir, str(self.most_recent_3d_id) + ".jpg"))
                    dest.append(os.path.join('walk', image_dir, img_file_name))

                diffs = diff_houchao.diff_2d(src, dest)
                avg_diff = round(np.average(diffs), 5)

                if (avg_diff >= self.th or self.most_recent_3d_id == 0) and self.timestamp not in self.reconstructed_ids:
                ...
                """
                if (self.timestamp - self.start) % 300 == 0:
                    print("###################################")
                    print("Remaining tasks =", self.end - self.start - self.total_task)
                    print("Current queue =", self.quality_queue)
                    print(f"current v/w = {self.v}/{self.window}")

                    self.v, self.window = config_v_w(self.quality_req, self.total_task, self.total_task_q,
                                                     self.end - self.start - self.total_task, self.quality_queue,
                                                     self.server_paras, self.diff_th, self.f1_res, self.f1_d)

                    self.init_tp = self.window / self.dd_speed
                    print("###################################")
                """
            else:
                time.sleep(0.1)
            ...
            if len(self.reconstructed_ids) > 20:
                self.DD_stop = True
                print("DD stop, task size =", len(self.reconstructed_ids))
                time.sleep(0.1)
            else:
                self.DD_stop = False
            """
            ...
            if self.delivery_task or (self.init and time.time() - start_t >= self.init_tp):
                ...
                ###########################################
                print(logger() + f"enter new TP, total tasks = {total_task}, {self.online_start_timestamp}-{self.online_end_timestamp}")
            ...
            """
            if timestamp - self.online_start_timestamp <= self.window or False:
                timestamp = min(end_timestamp + 1, timestamp + self.interval)
            else:
                time.sleep(0.1)
            """
    ...
    def batch_processing(self, start, end, scheduled, assignments, resolution, th):
        ...
        """
        if len(self.long_term_f) > 0 and np.abs(self.long_term_f[-1] - self.quality_req) <= 0.025:
            while len(self.reconstructed_ids) <= 8 and self.timestamp < self.end and time.time() - start_t < self.init_tp:
                time.sleep(0.2)
        """

       # """

        while time.time() - start_t < 3: #Abr 5 Fix self.init_tp
            time.sleep(0.2)
        ...
        print(f"current TP F-score = {tp_quality}, TP accumulated = {round(tp_quality * (end - start), 4)}, quality queue = {round(self.quality_queue, 4)}")
